...
int _draw_particle(int id, float size, float x, float y, float z) {
    ...
    for(i=0; i<3; i++) {
        for(j=0; j<3; j++) {
            if (i==j) {
                model_view_matrix[i*4+j] = 1.0;
            } else {
                model_view_matrix[i*4+j] = 0.0;
            }
        }
    }
...
int _draw_particle2(int id, float size, float x, float y, float z) {
    ...
    float up[3] = {model_view_matrix[0], model_view_matrix[4], model_view_matrix[8]};
    float right[3] = {model_view_matrix[1], model_view_matrix[5], model_view_matrix[9]};
...
int _planar_laser2(int density, float width, float x0, float y0, float z0, float x1, float y1, float z1) {
    if(current_camera == NULL) {
        printf("particle_functions planar_laser2() camera is null \n");
        return 0;
    }
