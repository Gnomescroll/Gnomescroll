...
struct Vec3 Agent_state::enemy_flag_pos()
{
    struct Vec3 null = vec3_init(0.0, 0.0, 0.0);
    ...
    return vec3_init(f->x, f->y, f->z);
}

struct Vec3 Agent_state::friendly_base_pos()
{
    struct Vec3 null = vec3_init(0.0, 0.0, 0.0);
...
void Agent_state::get_spawn_point(int* spawn) {
    ...
    {    // check that assigned spawner still exists, reassign if not
        while ((s = STATE::spawner_list->get_by_team_index(this->status.team, this->status.spawner)) == NULL)
        {
