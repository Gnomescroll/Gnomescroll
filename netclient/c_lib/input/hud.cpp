...
// NULL if none match
static Inventory* get_inventory_slot_selection(int x, int y, int* slot)
{
    return NULL;
    ...
    // iterate inventories
    // get slot from render component
    // if slot < 0 ignore
    // else record depth of inventory
    // take closest with slot >= 0
    // return slot
}

// handlers need to dispatch to hud inventory and model inventory
// must request which icon was selected in hud
// then set it on the inventory model object
