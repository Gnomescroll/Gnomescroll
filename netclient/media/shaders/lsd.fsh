...
void main() 
{
    ...
    //color.r = snoise(vertex * vec3(1.0/3.0)) / 32.0f;
    //color.g = snoise(vertex * vec3(2.0/3.0)) / 32.0f;
    //color.b += snoise(vertex / 10.0f);    1D
    vec2 v2 = vec2(vertex[0], vertex[1]);
    float m = snoise(sin(v2));
    color.b += m;
