...
save</a>\
 <a href="javascript:save_by_color();">\
save color</a>\
...
function save_by_color() {

    linesMaterial.color.setRGBA( 0, 0, 0, 0 );
    brush.position.y = 2000;
    render();

    var dc_vox_obj = saveDCVoxByColor();
    var dc_vox_formatted = format_vox_obj(dc_vox_obj);
    displayJSONdata(dc_vox_formatted);
    ...
    for (i=0;i<len;i++)
    {
        ...
        for (j=0; j<clen; j++)
        {
            ...
            if (!(mat instanceof THREE.MeshColorFillMaterial))
                continue;
            if (mat.color.r == color.r && mat.color.g == color.g && mat.color.b == color.b)
            {
                match = true;
                break;
            }
...
{
    color = color || brush.material[0].color;
    var objs = filterSceneObjectsByColor(color);
    return saveDCVox(objs);
}
