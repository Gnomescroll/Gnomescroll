...
static void init_synthesizer_texture()
{
    ...
    int ret = create_texture_from_file("media/sprites/container/synthesizer48.png", &SynthesizerTexture, min_filter, mag_filter);
    GS_ASSERT(ret == 0);
...
static void init_crafting_texture()
{
    GLuint min_filter = GL_LINEAR;
    GLuint mag_filter = GL_NEAREST;
    int ret = create_texture_from_file("media/sprites/container/crafting_bench48.png", &CraftingTexture, min_filter, mag_filter);
    GS_ASSERT(ret == 0);
...
static void init_storage_block_texture()
{
    GLuint min_filter = GL_LINEAR;
    GLuint mag_filter = GL_NEAREST;
    int ret = create_texture_from_file("media/sprites/container/storage_block48.png", &StorageBlockTexture, min_filter, mag_filter);
    GS_ASSERT(ret == 0);
...
static void init_smelter_texture()
{
    GLuint min_filter = GL_LINEAR;
    GLuint mag_filter = GL_NEAREST;
    int ret = create_texture_from_file("media/sprites/container/smelter48.png", &SmelterTexture, min_filter, mag_filter);
    GS_ASSERT(ret == 0);
