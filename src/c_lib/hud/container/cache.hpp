...
class CacheUI : public UIElement
{
    public:
...
const float CacheUI::border = 0;       // border around entire panel
const float CacheUI::inc1 = 5; // spacing between slot icons
const float CacheUI::inc2 = 2;  // border around a slot icon
const float CacheUI::slot_size = 32;    // pixel dimension

int CacheUI::get_slot_at(int px, int py)
{
...
void CacheUI::draw()
{
    ...
    if (!isValid(this->container_id)) return;
