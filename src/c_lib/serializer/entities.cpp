...
static bool parse_agent_spawner_token(const char* key, const char* val, class ParsedAgentSpawnerData* data)
{
    ...
    if (strcmp(key, USER_COUNT_TAG) == 0)
    {
        ...
        GS_ASSERT(!err && user_count >= 0 && user_count <= Entities::MAX_USERS_PER_AGENT_SPAWNER);
        if (err || user_count < 0 || user_count > Entities::MAX_USERS_PER_AGENT_SPAWNER) return false;
