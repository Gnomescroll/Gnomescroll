...
class VoxelModel
{
    ...
    void set_skeleton_root(const Vec3& position, float theta);
    ...
    void update(const Vec3& position, float theta, float phi);
    ...
    bool in_sight_of(const Vec3& source, Vec3* sink);
    bool in_sight_of(const Vec3& source, Vec3* sink, float failure_rate);
