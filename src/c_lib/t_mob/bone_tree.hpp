...
/*
    ...
    for (size_t a = 0; a < mesh->mNumBones; ++a)
    {
      ...
      aiNode* node = find_node_recursively_by_name(scene->mRootNode, bone->mName);
...
*/
    ...
    aiNode* find_node_recursively_by_name(aiNode* pNode, char* node_name)
    {
        ...
        for (size_t i=0; i < pNode->mNumChildren; i++)
        {
            aiNode* tmp = find_node_recursively_by_name(pNode->mChildren[i], node_name);
    ...
    void draw_skeleton(float x, float y, float z)
    {
        ...
        for (int i=0; i<nli; i++)
        {
            ...
            for (unsigned int j=0; j<mesh->mNumBones; j++)
            {
                ...
                aiNode* node = find_node_recursively_by_name(pScene->mRootNode, bone->mName.data);
                GS_ASSERT(node != NULL)
                // start with the mesh-to-bone matrix

                Mat4 boneMatrix = _convert_matrix(bone->mOffsetMatrix);  //node to vertex matrix?
                ...
                */
                ...
                while (tempNode)
                {
                    ...
                    if (_print)
                    {
                        ...
                        //boneMatrices[a] *= tempNode->mTransformation;   // check your matrix multiplication order here!!!
                        //mat4_print(_convert_matrix(tempNode->mTransformation)) ;
                    }
                        //boneMatrix = mat4_mult(boneMatrix, _convert_matrix(tempNode->mTransformation));
                ...
                //boneMatrix = mat4_mult(mat4_transpose(_convert_matrix(bone->mOffsetMatrix)) , boneMatrix);
...
*/
    ...
    struct Mat4 get_anim_matrix(int frame_time, aiNodeAnim** node_channels, int node_channel_max, aiNode* node)
    {
        ...
        //if cannot find, then node does not have an animation and use this
        //return mat4_transpose(_convert_matrix(node->mTransformation));
        return _convert_matrix(node->mTransformation);
...
/*
        Mat4 sceneMatrix;
        {
            ...
            aiNode* node = find_node_recursively_by_name(pScene->mRootNode, "Scene");
            printf("!!! node: %s \n", node->mName.data);
            //mat4_print(mat4_transpose(_convert_matrix(node->mTransformation))) ;
            sceneMatrix = mat4_transpose(_convert_matrix(node->mTransformation));
        }
*/

        //Mat4 m_GlobalInverseTransform = mat4_inverse(mat4_transpose(_convert_matrix(pScene->mRootNode->mTransformation)));
        ...
        for (int i=0; i<nli; i++)
        {
            ...
            for (unsigned int j=0; j<mesh->mNumBones; j++)
            {
                ...
                while (tempNode)
                {
                    ...
                    boneMatrix = mat4_mult(get_anim_matrix(frame_time, node_channels, node_channels_max, tempNode), boneMatrix);
                    GS_ASSERT(boneMatrix._f[0*4+3] == 0.0f && boneMatrix._f[1*4+3] == 0.0f && boneMatrix._f[2*4+3] == 0.0f && boneMatrix._f[3*4+3] == 1.0f);
                    //boneMatrix = mat4_mult(_convert_matrix(tempNode->mTransformation), boneMatrix);
                ...
                //node = find_node_recursively_by_name(pScene->mRootNode, bone->mName.data);
